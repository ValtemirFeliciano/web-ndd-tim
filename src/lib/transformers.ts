...
export function calcularAreaBase(
  ...
  logDebug("info", `Entrada: nx_base="${nxBase}", di_base="${diBase}"`);
  ...
  logDebug("info", `Quantidade de bases: ${quantidade}`);
  ...
  logDebug("info", `Dimensões extraídas do PDF: "${dimensaoLimpa}"`);
  ...
  logDebug("info", `Dimensão 1: ${dim1}m, Dimensão 2: ${dim2}m`);

  if (isNaN(dim1) || isNaN(dim2)) {
    logDebug("warn", `Valores inválidos (dim1=${dim1}, dim2=${dim2})`);
    return "";
  }

  // Calcular dimensões totais (quantidade × dimensões)
  const total1 = quantidade * dim1;
  const total2 = quantidade * dim2;
  logDebug("info", `Cálculo: ${quantidade} × ${dim1} = ${total1}m, ${quantidade} × ${dim2} = ${total2}m`);
  ...
  logDebug("info", `Resultado final: "${resultado}"`);
  logDebug("info", `═══════════════════════════════`);
